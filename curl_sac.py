...
class CurlSacAgent(object):
    ...
    def __init__(
        ...
        self.log_alpha = torch.tensor(np.log(init_temperature)).to(device=torch.device('cuda:0'))
        ...
        if self.encoder_type == 'pixel':
            ...
            self.CURL = CURL(obs_shape, encoder_feature_dim,
                        self.curl_latent_dim, self.critic,self.critic_target, output_type='continuous').to(device=torch.device('cuda:0'))
    ...
    def select_action(self, obs):
        with torch.no_grad():
            obs = torch.FloatTensor(obs).to(device=torch.device('cuda:0'))
            obs = obs.unsqueeze(0)
            mu, _, _, _ = self.actor(
                obs, compute_pi=False, compute_log_pi=False
            )
            return mu.cpu().argmax().item()
